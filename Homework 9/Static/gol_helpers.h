...
void find_nbrs(gol_board* g, int i, int j, int* nbr_r, int* nbr_c,int* numnbrs){
  ...
  *numnbrs=0;
  ...
  int l;
  for(l=0;l<8;l++){
    if (i+pr[l]>=0 && i+pr[l]<=g->m-1 && j+pc[l]>=0 && j+pc[l] <=g->n-1){
      nbr_r[*numnbrs]=i+pr[l];
      nbr_c[*numnbrs]=j+pc[l];
      *numnbrs=*numnbrs+1;
    }
  }
...
void printusage(){
  ...
  fprintf(stderr,"-i=inputfile     Use this input file.  Default is blinker.txt\n");
  fprintf(stderr,"-o=outputfile    Use this output file. Default is to derive output filename from input filename.\n");
  fprintf(stderr,"-g=g             Use g generations. Default is 100.\n");
...
options parse_commandline(int argc,char** argv){
  options o;
  strcpy(o.inputfile,"blinker.txt");
...
void insert_rect(unsigned char * pixels, int m, int n, int top_left_r, int top_left_c, int bottom_right_r, int bottom_right_c,int colorindex){
  ...
  for (i=top_left_r; i<=bottom_right_r;i++){
    ...
    memset(pixels+i*m+top_left_c, colorindex, bottom_right_c-top_left_c+1);
...
void gol_board_to_gif(gol_board* g, options o){
  ...
  for (t=0;t<g->generations;t+=frameskip){
    ...
    for (i=0;i<g->m;i++){
      for (j=0;j<g->n;j++){
	if (i>0) insert_rect(pixels,o.resolution[0],o.resolution[1],sqr_top_row[i],sqr_left_col[j],sqr_top_row[i]+2,sqr_right_col[j],1);
	if (j>0) insert_rect(pixels,o.resolution[0],o.resolution[1],sqr_top_row[i],sqr_left_col[j],sqr_bottom_row[i],sqr_left_col[j]+2,1);
	if (i<g->m-1) insert_rect(pixels,o.resolution[0],o.resolution[1],sqr_bottom_row[i]-2,sqr_left_col[j],sqr_bottom_row[i],sqr_right_col[j],1);
	if (j<g->n-1) insert_rect(pixels,o.resolution[0],o.resolution[1],sqr_top_row[i],sqr_right_col[j]-2,sqr_bottom_row[i],sqr_right_col[j],1);

	if (get_status(g,i,j,t)==OCCUPIED) insert_rect(pixels,o.resolution[0],o.resolution[1],sqr_top_row[i]+2,sqr_left_col[j]+2,sqr_bottom_row[i]-2,sqr_right_col[j]-2,2);
      }
    }
